...
if [ $# -lt 1 ]; then
    ...
    echo "ex. $ $0 null"
    echo
    echo "    [RUNNING_TYPE] : DB connection information to get parameters"
    echo "                     \"docker\" - Run with docker-compose"
    echo "                     \"watch\" - idk"
    echo "                     \"inc\" - bundle upate before running"
    echo "                     \"null\" - idk"
    echo "                     \"back\" - Running in background"
    echo "                     \"kill\" - Kill running process with pid in .localhost.pid"
    exit 1
fi
...
if [[ $1 = "docker" ]]; then
    ...
    bundle exec jekyll serve --port 4001 --incremental --trace >> .localhost.log 2>&1 &
